...
import {Orchestrator} from "./base/Orchestrator.sol";
import {IthacaAccountV2} from "./IthacaAccountV2.sol";
import {LibEIP7702} from "solady/accounts/LibEIP7702.sol";
import {Simulator} from "./base/Simulator.sol";
import {ZKEmailVerifier} from "./ZKEmailVerifier.sol";

/// @title IthacaFactoryV2
/// @notice Factory contract for deterministic deployment of Ithaca Account V2 contracts using CREATE2
/// @dev Deploy this factory first via Safe Singleton Factory, then use it to deploy all other contracts
contract IthacaFactoryV2 {
    // Custom errors
    ...
    /// @notice Deploys the ZKEmailVerifier contract with CREATE2
    function deployZKEmailVerifier(bytes32 salt) public returns (address) {
        bytes memory bytecode = type(ZKEmailVerifier).creationCode;
        return _deploy(bytecode, salt);
    }

    /// @notice Deploys the IthacaAccountV2 implementation contract with CREATE2
        ...
        returns (address)
    {
        ...
        bytes memory bytecode =
            abi.encodePacked(type(IthacaAccountV2).creationCode, abi.encode(orchestrator));
        return _deploy(bytecode, salt);
    }
        ...
        )
    {
        ...
        zkEmailVerifier = deployZKEmailVerifier(salt);
    ...
    /// @notice Predicts the ZKEmailVerifier deployment address
    function predictZKEmailVerifierAddress(bytes32 salt)
        external
        view
        returns (address)
    {
    ...
    /// @notice Predicts the IthacaAccountV2 implementation deployment address
        ...
        )
    {
        ...
        zkEmailVerifier = this.predictZKEmailVerifierAddress(salt);
